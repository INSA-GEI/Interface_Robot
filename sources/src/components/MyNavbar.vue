<template>
  <v-toolbar dark color="pink darken-3">
    ...
    <v-tooltip>
    <img id="logo-battery" src="static/battery_medium_white.svg" slot="activator" alt="">
...
export default {
  ...
  sockets:{
    BAT: function(data){
      bat_logo = document.getElementById('logo-baattery');
      if(data === '0'){
        logo.source = "static/battery_high_white.svg";
      }else if (data === '1'){
        logo.source = "static/battery_medium_white.svg";
      }else{
        logo.source = "static/battery_low_white.svg";
      }
    },
